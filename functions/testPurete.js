const clickTESTPURETE = document.getElementById('linkTESTPURETE')

document.addEventListener('click', function (e) {
    if (e.target && e.target.id == 'linkTESTPURETE') {
        while (document.body.lastElementChild) {
            document.body.removeChild(document.body.lastElementChild);
        }

        var textExplicationImg = 'Page d\'accueil du test'
        var txtExplicationProjet = 'Ce projet consistait à coder un test de pureté comme on peut en retrouver sur Internet ' +
            'avec des questions assez malaisantes, la plupart concernant directement mes amis. A la fin du jeu, ' +
            'le score de la personne est affichée, plus le score tend vers le négatif, moins la personne est "pure".'
        ...
        var txtProjects = document.createTextNode('TEST DE PURETÉ')
        ...
        img1.src = './images/TEST_PURETE/Test_Purete_Presentation.PNG'

        var aImg1 = document.createElement('a')
        aImg1.href = './images/TEST_PURETE/Test_Purete_Presentation.PNG'

        var img2 = document.createElement('img')
        img2.id = 'gsbjava1'
        img2.className = 'imgProject'
        img2.src = './images/TEST_PURETE/Test_Purete_Question.PNG'

        var aImg2 = document.createElement('a')
        aImg2.href = './images/TEST_PURETE/Test_Purete_Question.PNG'

        var img3 = document.createElement('img')
        img3.id = 'gsbjava2'
        img3.className = 'imgProject'
        img3.src = './images/TEST_PURETE/Test_Purete_Score_Negatif.PNG'

        var aImg3 = document.createElement('a')
        aImg3.href = './images/TEST_PURETE/Test_Purete_Score_Negatif.PNG'

        var img4 = document.createElement('img')
        img4.id = 'gsbjava3'
        img4.className = 'imgProject'
        img4.src = './images/TEST_PURETE/Test_Purete_Score_Positif.PNG'

        var aImg4 = document.createElement('a')
        aImg4.href = './images/TEST_PURETE/Test_Purete_Score_Positif.PNG'
        ...
        nextArrow.addEventListener("click", () => {
            ...
            switch (i + 1) {
                case 1:
                    textExplicationImg = 'Exemple de question du test'
                    txtExpImg = document.createTextNode(`${textExplicationImg}`)
                    pTxtExpImg.innerHTML = ''
                    pTxtExpImg.appendChild(txtExpImg)
                    break;
                case 2:
                    textExplicationImg = 'Affichage d\'un score négatif'
                    txtExpImg = document.createTextNode(`${textExplicationImg}`)
                    pTxtExpImg.innerHTML = ''
                    pTxtExpImg.appendChild(txtExpImg)
                    break;
                case 3:
                    textExplicationImg = 'Affichage d\'un score positif'
                    txtExpImg = document.createTextNode(`${textExplicationImg}`)
                    pTxtExpImg.innerHTML = ''
                    pTxtExpImg.appendChild(txtExpImg)
                    break;
                case 4:
                    textExplicationImg = 'Page d\'accueil du test'
                    txtExpImg = document.createTextNode(`${textExplicationImg}`)
                    pTxtExpImg.innerHTML = ''
                    pTxtExpImg.appendChild(txtExpImg)
                    break;
            ...
            if (i < images.length - 1) {
                const testPurete = document.getElementById("gsbjava" + i);
                testPurete.className = "imgCurrent"
                i++
                testPurete.className = 'imgProject'
                const testPurete1 = document.getElementById("gsbjava" + i);
                testPurete1.className = "imgCurrent"
            } else {
                const testPurete1 = document.getElementById("gsbjava" + i);
                testPurete1.className = "imgProject"
                i = 0
                const testPurete = document.getElementById("gsbjava" + i)
                testPurete.className = "imgCurrent"
            }
